...
for thisTrial_2 in trials_2:
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    if gotValidClick:  # abort routine on response
                        continueRoutine = False
        # Run catch trials 
        if not ((trialnumber == catchtrialorderprac[0]) or (trialnumber == catchtrialorderprac[1])):
            continueRoutine=False
    ...
    gotValidClick = False  # until a click is received
    pracnumbertext = f'You have finished {pracnumber} of 9 practice questions';
    
    
    text_31.setText(pracnumbertext)
    # keep track of which components have finished
    summary_pracComponents = [response1disk_6, response2disk_6, response3disk_6, response4disk_6, response5disk_6, response6disk_6, response7disk_6, response8disk_6, mouse_6, rectangle_4, text_30, text_31]
